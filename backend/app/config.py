...
class Settings(BaseSettings):
    ...
    #
    # 창이 없으면 외부가 조회를 잃어버렸을 때(404 등) 완료될 수 없는 pending이
    # 그 직원의 신규 조회를 영원히 막는 교착이 된다.
    #
    # 5분은 실측 완료 시간(~10초)의 30배다. 진짜 처리 중인 조회를 오판할
    # 여유는 충분하면서, 교착에 걸린 관리자가 기다리는 시간은 짧다.
    # 이 둘이 트레이드오프이며 여유를 늘리면 대기도 늘어난다.
    CHECK_IN_PROGRESS_WINDOW_SECONDS: int = 300
