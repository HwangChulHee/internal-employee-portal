...
프론트도 이 간극을 안다. 생성 응답이 `pending`이 아니면 **곧바로 GET을 한 번 보내**
백엔드가 동기화한 완전한 레코드를 받아 표시한다. 이것이 없으면 "이상 없음" 배지 아래
모든 세부 항목이 "확인 중"으로 남는 자기모순 화면이 된다.
폴링 조건 자체는 `pending` 하나로 유지한다. `completed_at`이 비어 있으면 폴링하는 식으로
넓히면, 외부 API가 완료 응답에 `completedAt`을 주지 않는 경우 폴링이 무의미하게
10회 반복된다. 응답이 무작위인 이 API에서 그 위험을 배제할 수 없었다.
