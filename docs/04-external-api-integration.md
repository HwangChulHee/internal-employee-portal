...
프론트도 이 간극을 안다. 표시 규칙과 폴링 조건이 같은 판정을 쓴다:
**완결(status 확정 + `completedAt` 존재) 전에는 실제 status가 무엇이든 "조회 중"으로
표시하고, 완결될 때까지 폴링한다.** 생성 직후의 요약(판정만 있고 세부 없음)이나
동기화가 실패해 남은 반동기화 레코드가 "추가 검토 필요인데 완료시각이 없는" 모순
화면으로 노출되지 않고, 판정은 세부 결과·완료시각과 함께 한 번에 나타난다.
외부가 `completedAt`을 끝내 주지 않는 경우의 무한 폴링은 최대 횟수(10회)가 막는다.
소진되면 "조회가 진행 중입니다" 안내와 [다시 확인] 버튼이 남는다.

응답이 끝내 오지 않으면 시간 창(기본 1시간)을 넘기며 **"응답 없음"**으로 표시가 바뀌고
새 조회를 요청할 수 있게 된다. "실패"라고 적지 않는다 — 외부에 실패 상태가 없어
실패 여부를 알 수 없고, 뒤늦게 완료되면 판정으로 바뀐다
(`05-data-model.md`의 "상태 동기화" 참조).
...
### 예외: 폴링의 동기화 GET은 1회만 시도한다

상세 조회가 미완결 건을 동기화할 때의 외부 GET은 **재시도 없이 1회**다.
실패하면 즉시 로컬 값을 반환하고 다음 폴링에 맡긴다.

버티는 재시도는 다시 기회가 없는 요청(생성)의 도구다. 이 경로는 폴링이
3초마다 다시 오므로 재시도 루프가 이미 바깥에 있고, 안에서 또 30초씩
(`retryAfter`) 버티면 GET 하나가 1분 가까이 열려 프론트가 그동안 응답을
받지 못한다. 실제로 조회 요청 후 화면이 2분 넘게 굳는 사고의 원인이었다.
"짧게 끊고 다음 폴링에 맡긴다"는 타임아웃 원칙과 같은 판단이다.

| | 생성(POST) | 동기화(GET) |
|---|---|---|
| 실패 시 다음 기회 | 없다 — 사용자가 다시 눌러야 한다 | 3초 뒤 폴링이 온다 |
| 재시도 | 총 3회 시도 | **1회 시도** |
| 실패 처리 | 503/504 에러 응답 | 로컬 값 반환 (조회는 성공) |
...
| 창(1시간) 초과 | 외부 응답이 오랫동안 없습니다. 새 조회를 요청할 수 있고, 이 조회는 응답이 오면 채워집니다. |
...
|---|---|
| HTTP 타임아웃 | 10초 |
| 재시도 | 총 3회 시도 (재시도 2회) · 폴링 동기화 GET만 1회 |
| 500 백오프 | 1s → 2s |
| 503 대기 | 응답의 `retryAfter` |
| 폴링 간격 | 3초 |
| 폴링 최대 횟수 | 10회 |
| 진행 중 판정 창 | 1시간 (`CHECK_IN_PROGRESS_WINDOW_SECONDS`) |
