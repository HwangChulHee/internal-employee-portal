...
(`CHECK_IN_PROGRESS_WINDOW_SECONDS`, 기본 5분) 안이면 `in_progress: true`다.
응답에 담아 내려보내므로 프론트가 같은 창을 따로 들지 않는다.

창 길이는 트레이드오프다. 길수록 진짜 처리 중인 조회를 "응답 없음"으로 오판할 위험이
줄지만, 교착에 걸린 관리자가 새 조회를 요청하기까지 그만큼 기다린다.
5분은 실측 완료 시간(~10초)의 30배로, 오판 여유를 확보하면서 대기를 짧게 잡은 값이다.
