...
**처리 방식**: 상세 조회 시 로컬 상태가 `pending`이면 외부 API에 재확인하고 결과를 갱신한다.
...
관리자가 계정을 생성할 때 **로그인 아이디와 동일한 값**을 초기 비밀번호로 설정한다.
관리자가 별도로 비밀번호를 정하거나 전달할 필요가 없어 계정 생성 흐름이 단순해진다.

다만 이 방식은 아이디를 아는 사람이 곧 비밀번호를 아는 것과 같으므로,
실제 운영 환경이라면 최초 로그인 시 비밀번호 변경을 강제해야 한다.
이를 도입하려면 `must_change_password BOOLEAN NOT NULL DEFAULT true` 컬럼과
변경 화면·리다이렉트 흐름이 추가로 필요하다.

현재는 과제 범위를 고려해 도입하지 않으며, 이 한계를 README에 명시한다.
