...
관리자 → "직원 등록" → 새 직원을 만든다. 비밀번호 입력란이 없음을 확인한다.
등록 후 안내 문구를 읽고, 그 아이디로 로그인해본다.

이어서 **같은 사번으로 또 등록**을 시도한다.

### 관찰하기

- Network 탭: `POST /api/employees`, 요청에 비밀번호 필드가 없다
- 생성된 계정의 초기 비밀번호 = 아이디
- 사번 중복 시 409 "이미 사용 중인 사번입니다"
- 아이디 중복 시 409 "이미 사용 중인 아이디입니다" (둘이 구분된다)
...
POST /api/employees
  → services/employee_service.py: create_employee()
      1. 사번·아이디 사전 조회 (있으면 409)
      2. 초기 비밀번호 = login_id, bcrypt 해싱
      3. INSERT
      4. IntegrityError 발생 시에도 409로 변환 (경쟁 조건 대비)
```

### 설계 포인트

**초기 비밀번호 = 아이디**: 관리자가 비밀번호를 따로 정하거나 전달할 필요가 없다.
다만 보안 한계가 있어 실운영이라면 최초 로그인 시 변경을 강제해야 한다.
