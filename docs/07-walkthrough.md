...
| `emp004` | 정하윤 | 직원 | 재직 | 목록 페이징 |
| `emp005` | 최지우 | 직원 | 재직 | 목록 페이징 |
| `emp006` | 박서준 | 직원 | 재직 | 목록 페이징 |
| `emp007` | 황민서 | 직원 | 재직 | 황씨 — 복성 아님 확인 |
| `emp008` | 한지민 | 직원 | **퇴사** | 퇴사 필터 + 페이징 |
| `emp009` | 박퇴사 | 직원 | **퇴사** | **접근 차단** |

목록은 10명 단위로 페이징된다. 시드 10명은 정확히 한 페이지라 페이저가
숨겨져 있고, 직원을 하나 등록하는 순간 "1 / 2 페이지"가 나타난다.
신원조회 이력은 5건 단위다.
...
## 4. 직원 목록 — 응답 스키마 분리와 페이징

### 해보기

`admin`으로 로그인 → 직원 목록을 본다. 필터(전체/재직/퇴사)와 검색,
시드 10명은 한 페이지에 다 들어가 페이저가 없다. **직원을 하나 등록한 뒤**
[이전]/[다음]으로 페이지를 넘겨보고, **2페이지에서 필터를 바꿔본다.**

### 관찰하기

- Network 탭: `GET /api/employees?page=1&page_size=10`, 응답이
  `{ items, total, page, page_size }` 골격이다. items에는 **생년월일이 없다**
- `?status=ACTIVE`, `?q=남궁` 붙는 것 확인 — 필터와 페이징이 함께 간다
- 2페이지에서 필터·검색을 바꾸면 **1페이지로 돌아간다**.
  조건이 바뀌었는데 3페이지에 머물면 빈 화면이 나오기 때문이다
- 퇴사자(박퇴사, 한지민)도 목록에 나온다 — 기본값이 전체다
- 상세를 클릭하면 `GET /api/employees/{id}` — 여기엔 생년월일이 있다

### 흐름

```
목록: GET /api/employees?page=&page_size=
  → response_model=EmployeePage      (items + total, 생년월일 없음)
...
**오프셋 페이징이면 충분하다.** total을 함께 내려 프론트가 "n / m 페이지"를
