...
export interface PollingState {
  check: BackgroundCheckDetail | null
  polling: boolean
...
export function useCheckPolling(
  ...
  const [check, setCheck] = useState<BackgroundCheckDetail | null>(target)
  ...
  const recheck = useCallback(() => {
    ...
    void (async () => {
      try {
        setCheck(await checksApi.getCheck(targetId))
      } catch {
  ...
  return { check, polling, exhausted, rechecking, error, recheck }
}
